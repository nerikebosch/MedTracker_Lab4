...
class MedicationModelTests(TestCase):
    ...
    def test_adherence_rate_no_logs(self):
        med = Medication.objects.create(
            name="Placebo", dosage_mg=0, prescribed_per_day=1
        )
        adherence = med.adherence_rate()
    ...
    def test_fetch_external_info_failure(self, mock_get_info):
        med = Medication.objects.create(
            name="FailingDrug", dosage_mg=10, prescribed_per_day=1
        )

        test_exception_message = (
            "External service validation failed: Missing drug name."
        )
    ...
    def test_str_dose_log_returns_name_status_and_when(self):
        ...
        dose_log = DoseLog.objects.create(
            medication=med, taken_at=taken_time, was_taken=True
        )

        self.assertEqual(
            str(dose_log),
            f"Aspirin at {timezone.localtime(taken_time).strftime('%Y-%m-%d %H:%M')} - Taken",
        )
...
class NoteModelTests(TestCase):
    def test_create_note_success(self):
        med = Medication.objects.create(
            name="Oxycodone", dosage_mg=10, prescribed_per_day=2
        )
        from medtrackerapp.models import Note

        note = Note.objects.create(
            medication=med, text="Patient reported dizziness.", created_at=date.today()
        )
    ...
    def test_str_returns_text_preview(self):
        med = Medication.objects.create(
            name="Tylenol", dosage_mg=500, prescribed_per_day=4
        )
        from medtrackerapp.models import Note

        note = Note.objects.create(
            medication=med, text="Short note", created_at=date.today()
        )
